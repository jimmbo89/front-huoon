  ...
  <v-container class="pa-4" >
    <v-card elevation="2" rounded="lg" flat>
      ...
      <v-card-actions class="pa-3 bg-grey-lighten-5 tools-bar mt-2">
        ...
        <v-col cols="12">
          <ProductMovement
            v-if="currentView === 'movement' && warehouseData"
            :key="`movement-${warehouseData.id}`"
            :warehouseData="warehouseData"
            @warehouse-updated="$emit('warehouse-updated', $event)"
          />
          <ActivityLogs
...
import ProductMovement from "../product/ProductMovement";
import ActivityLogs from "../product/ActivityLogs";
export default {
  props: {
      warehouses: {
      type: Array,
  ...
  created() {
    this.tools = [
      {
        name: this.$t("activity.title"),
        action: () => this.showActivities(),
      },
    ];
  },
  mounted() {
