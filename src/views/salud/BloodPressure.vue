    ...
    <v-card>
      <v-toolbar color="#DA7171">
        <span class="text-subtitle-2 ml-4">
          {{
            $t("deleteDialog.title", { item: $t(`deleteDialog.items.blood_pressure`) })
          }}</span
        >
      </v-toolbar>
...
export default {
  ...
  data: () => ({
    ...
    editedIndex: -1,
    headers: [
      { title: "Presión Arterial", key: "blood_pressure" },
    ...
    weightRules: [(v) => !v || (v > 0 && v < 300) || "Peso debe ser entre 0-300 kg"],
  ...
  computed: {
    ...
    formTitle() {
      return this.editedIndex === -1
        ? this.$t("physicalExam.forms.blood_pressure.title.add")
        : this.$t("physicalExam.forms.blood_pressure.title.edit");
    },
    getDate() {
  ...
  created() {
    this.tools = [
      {
        name: this.$t("physicalExam.forms.blood_pressure.title.add"),
        action: () => this.showAdd(),
      },
    ];
  },
  ...
  parseDateString(dateString) {
    ...
    async initialize() {
      ...
      this.data.query = "blood_pressure";
