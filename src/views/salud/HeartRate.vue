    ...
    <v-card>
      <v-toolbar color="#DA7171">
        <span class="text-subtitle-2 ml-4">
          {{
            $t("deleteDialog.title", { item: $t(`deleteDialog.items.pulse`) })
          }}</span
        >
      </v-toolbar>
  ...
  computed: {
    formTitle() {
      return this.editedIndex === -1
        ? this.$t("physicalExam.forms.pulse.title.add")
        : this.$t("physicalExam.forms.pulse.title.edit");
    },
  },
  created() {
    this.tools = [
      {
        name: this.$t("physicalExam.forms.pulse.title.add"),
        action: () => this.showAdd(),
      },
    ];
  },
