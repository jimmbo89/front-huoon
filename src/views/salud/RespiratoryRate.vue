                ...
                >
                  ...
                  <div style="width: 83%; min-width: 0" class="d-flex flex-column">
                    <div class="text-body-2 text-truncate">
                      {{ slotProps.item.respiratory_rate }}
                    </div>
        ...
        <v-row dense>
          ...
          >
            <template v-slot:append>
              <span class="text-caption text-grey">rpm</span>
            </template>
          </v-text-field>
          </v-col>
          <!-- Campo: Fecha -->
    ...
    <v-card>
      <v-toolbar color="#DA7171">
        <span class="text-subtitle-2 ml-4">
          {{
            $t("deleteDialog.title", { item: $t(`deleteDialog.items.respiratory_rate`) })
          }}</span
        >
      </v-toolbar>
...
export default {
  ...
  data: () => ({
    ...
    editedIndex: -1,
    headers: [
      { title: "Frecuencia Respiratoria", key: "bloodrespiratory_rate" },
  ...
  created() {
    this.tools = [
      {
        name: this.$t("physicalExam.forms.respiratory_rate.title.add"),
        action: () => this.showAdd(),
      },
    ];
  },
  ...
  // Método para convertir string a Date (solo cuando sea necesario)
    ...
    async initialize() {
      ...
      this.data.query = "respiratory_rate";
