    ...
    <v-card>
      <v-toolbar color="#DA7171">
        <span class="text-subtitle-2 ml-4">
          {{
            $t("deleteDialog.title", { item: $t(`deleteDialog.items.temperature`) })
          }}</span
        >
      </v-toolbar>
...
export default {
  ...
  data: () => ({
    ...
    editedIndex: -1,
    headers: [
      { title: "Temperatura", key: "temperature" },
  ...
  created() {
    this.tools = [
      {
        name: this.$t("physicalExam.forms.temperature.title.add"),
        action: () => this.showAdd(),
      },
    ];
  },
